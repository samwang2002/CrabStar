...
U64 generate_hash_key(board_state *board)
{
    U64 final_key = 0ULL; // final hash key
    U64 bitboard; // temp piece bitboard copy
    for (int piece = P; piece <= k; piece++)
    {
        bitboard = board->bitboards[piece]; //initialize piece bitboard copy
    ...
    if (board->side == black) final_key ^= side_key;
...
int read_shared_entry(board_state *board, int alpha, int beta, int depth, int ply)
{
...
void write_shared_entry(board_state *board, int score, int depth, int hash_flag, int ply)
{
    lockless_ht *hash_entry = &shared_ht[hash_key % hash_size];
